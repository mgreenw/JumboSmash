...
const sceneQuery = utils.scenes.map(scene => `liked_${scene} = false, swiped_${scene}_timestamp = NOW()`);

/**
 * @api {post} /api/relationships/unmatch
 *
 */
...
const handler = [
  asyncHandler(async (req: $Request) => {
    return unmatch(req.user.id, req.params.matchUserId);
  }),
];
